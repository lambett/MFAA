...
class ShuffleNet(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False, mode='fc'):
        if mode == 'crosskd':
            out, f3_pre = self.layer3(x)  # 8x8
        ...
        covout = f3.clone().detach()

        if is_feat:
            if preact:
                if mode == 'sdd':
                    return [f0, f1_pre, f2_pre, f3_pre, f4], x, patch_score
                return [f0, f1_pre, f2_pre, f3_pre, f4], out, covout
            else:
                if mode == 'sdd':
                    return [f0, f1, f2, f3, f4], x, patch_score
                return [f0, f1, f2, f3, f4], out, covout
        else:
            return out
