...
class CIFAR100InstanceSampleMFAA(CIFAR100InstanceSample):
    """
    ...
    def __getitem__(self, index):
        ...
        if self.opt.mixup_num == -1:
            ...
            # ---------------------------------------------------------------
            # # 匹配相同类别中最近的距离的三张图像
            # # calculate cosine similarity between img and all positive samples in self.cls_positive[target]
            # start = time.time()
            # cos_similarities = []
            ...
            # # select the indices of the 3 positive samples with the smallest cosine similarity values
            ...
            # start1 = time.time()
            # img_vector = img.reshape(-1)
            # pos_samples = np.array(self.data[self.cls_positive[target]])
            # pos_samples_vector = pos_samples.reshape(pos_samples.shape[0], -1)
            # cos_similarities = np.dot(pos_samples_vector, img_vector) / \
            #                    (np.linalg.norm(pos_samples_vector, axis=1) * np.linalg.norm(img_vector))
            # # select the indices of the 3 positive samples with the smallest cosine similarity values
            # sorted_indexes = np.argsort(cos_similarities)[:3]
            # mixup_indexes = [self.cls_positive[target][i] for i in sorted_indexes]
            ...
            # euclidean_distances = []
            # img_vector = img.reshape(-1)
            # for pos_sample in self.cls_positive[target]:
            #     pos_sample_vector = pos_sample.reshape(-1)
            #     euclidean_distances.append(np.linalg.norm(img_vector - pos_sample_vector))
            #
            # # select the indices of the 3 positive samples with the smallest Euclidean distance
            # sorted_indexes = np.argsort(euclidean_distances)[:3]
            # mixup_indexes = [self.cls_positive[target][i] for i in sorted_indexes]
...
def get_cifar100_dataloaders_sample(batch_size=128, num_workers=3, k=4096,
    ...
    if loader_type == "mfaa":
        train_set = CIFAR100InstanceSampleMFAA(root=data_folder,
                                               download=True,
                                               train=True,
                                               transform=train_transform,
                                               k=k,
